...
// FIX: Add 'initialSelectedId' prop to handle pre-selection from routing.
const FeedbackHub: React.FC<{ 
...
}> = ({ onViewInsight, navigateTo, initialSelectedId }) => {
    ...
    // FIX: Add useEffect to handle setting the selected feedback item when the component is loaded with an ID.
    useEffect(() => {
        if (initialSelectedId) {
            const item = feedbackClusters.find(fb => fb.id === initialSelectedId);
            setSelectedFeedback(item || null);
        }
    }, [initialSelectedId, feedbackClusters]);
    ...
    const columns = useMemo((): Column<Feedback>[] => [
        {
            ...
            Cell: ({ value, row }: { value: string, row: { original: Feedback }}) => (
                <div className="font-medium text-text-primary max-w-xs">
                    {row.original.isNew && <span className="inline-block h-2 w-2 rounded-full bg-primary mr-2 animate-pulse"></span>}
                    {value}
                </div>
            )
        },
            ...
            Cell: ({ value }: { value: string[] }) => (
                <div className="flex flex-col items-start gap-1">
                    {value.map(pid => {
                        ...
                        return (
                            <button key={pid} onClick={(e) => { e.stopPropagation(); navigateTo(`product/detail/${pid}`) }} className="text-xs bg-background px-2 py-1 rounded-full hover:bg-primary/50 flex items-center">
                        ...
                        renderActions={() => (
                             <div className="flex items-center space-x-4">
                                <div className="flex items-center space-x-2">
                                    ...
                                    <button onClick={() => navigateTo('feedback/board')} className="px-3 py-1 text-sm rounded-md hover:bg-zinc-800">Board</button>
                                </div>
                                <button onClick={handleAddNew} className="px-4 py-2 rounded-md bg-primary text-white hover:bg-primary-hover">Add New Cluster</button>
                            </div>
                        )}
                    />
                </main>
