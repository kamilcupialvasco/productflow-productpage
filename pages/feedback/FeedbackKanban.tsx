...
const FeedbackCard: React.FC<{ item: Feedback; onClick: () => void }> = ({ item, onClick }) => (
    <div onClick={onClick} className="bg-background p-3 rounded-md border border-border/50 cursor-pointer hover:border-primary transition-all">
        ...
        <p className="text-xs text-text-secondary mt-1 line-clamp-2">{item.problemDescription}</p>
        <div className="flex justify-between items-center mt-3">
             <span className="text-xs font-mono px-1.5 py-0.5 bg-border rounded">{item.impact} impact</span>
                ...
                <main className="flex-1 p-8 overflow-x-auto">
                    <div className="flex justify-end mb-4">
                        <div className="flex items-center space-x-2">
                            ...
                            <button onClick={() => navigateTo('feedback/hub')} className="px-3 py-1 text-sm rounded-md hover:bg-zinc-800">Table</button>
                            <button className="px-3 py-1 text-sm rounded-md bg-primary text-white">Board</button>
                        </div>
                    </div>
                    <div className="flex space-x-6 h-full">
                        {columns.map(col => (
                            <div key={col} className="w-80 flex-shrink-0">
                                <h3 className="text-lg font-semibold text-text-primary mb-4 px-2">{col} ({feedbackClusters.filter(d=>d.status === col).length})</h3>
                                <div className="space-y-3 p-2 rounded-lg bg-card/50 h-[calc(100%-4rem)] overflow-y-auto">
                                    {feedbackClusters
