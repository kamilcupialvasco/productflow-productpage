...
import InsightsTable from '../../components/feedback/InsightsTable';
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center">
            <div className="bg-card rounded-lg w-full max-w-lg">
                <div className="p-4 border-b border-border"><h3>Manually Import Insight</h3></div>
                <div className="p-6 space-y-4">
                    <textarea value={content} onChange={e => setContent(e.target.value)} placeholder="Enter user feedback here..." className="w-full bg-background p-2 h-32 rounded"/>
                    <select value={productId} onChange={e => setProductId(e.target.value)} className="w-full bg-background p-2 rounded">
                 ...
                 <div className="p-4 flex justify-end space-x-2">
                    ...
                    <button onClick={handleSave} className="px-4 py-2 rounded bg-primary text-white">Save Insight</button>
                 </div>
            </div>
        </div>
    );
...
const FeedbackDashboard: React.FC<FeedbackDashboardProps> = ({ onViewInsight, navigateTo }) => {
  ...
  const metrics = useMemo(() => {
    ...
    const avgTriage = triageTimes.length > 0 ? triageTimes.reduce((a, b) => a + b, 0) / triageTimes.length : 0;
    const avgAction = actionTimes.length > 0 ? actionTimes.reduce((a, b) => a + b, 0) / actionTimes.length : 0;
    ...
    <div className="flex-1 flex flex-col overflow-y-auto">
      ...
      <div className="p-8 space-y-8">
        <div className="flex justify-end">
            <button onClick={() => setIsImporting(true)} className="px-4 py-2 rounded-md bg-primary text-white hover:bg-primary-hover">Manual Import</button>
        </div>
        ...
        <div>
          <h3 className="text-xl font-semibold mb-4 text-text-primary">Recent Insights</h3>
          <InsightsTable 
            data={insightData} 
