...
import InitiativeDetailModal from '../../components/upstream/InitiativeDetailModal';
import InitiativesTable from '../../components/upstream/InitiativesTable';
...
const InitiativeCard: React.FC<{ item: UpstreamInitiative, onClick: () => void }> = ({ item, onClick }) => (
    <div onClick={onClick} className="bg-background p-3 rounded-md border border-border/50 cursor-pointer hover:border-primary transition-all duration-200 shadow-sm hover:shadow-lg">
        ...
        <div className="flex justify-between items-center mt-3">
             <span className="text-xs font-mono px-1.5 py-0.5 bg-border rounded">{item.jiraId || 'No Jira ID'}</span>
            ...
            <div className="p-8 pb-0">
                <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center space-x-2">
                        ...
                        <button onClick={() => setViewMode('board')} className={`px-3 py-1 text-sm rounded-md ${viewMode === 'board' ? 'bg-primary text-white' : 'hover:bg-zinc-800'}`}>Board</button>
                        <button onClick={() => setViewMode('table')} className={`px-3 py-1 text-sm rounded-md ${viewMode === 'table' ? 'bg-primary text-white' : 'hover:bg-zinc-800'}`}>Table</button>
                    </div>
                    <button onClick={handleAddNew} className="px-4 py-2 rounded-md bg-primary text-white hover:bg-primary-hover">Add New Initiative</button>
                </div>
            </div>
                    ...
                    <div className="flex space-x-6">
                        {columns.map(col => (
                            <div key={col} className="w-80 flex-shrink-0">
                                <h3 className="text-lg font-semibold text-text-primary mb-4 px-2">{col} ({upstreamInitiatives.filter(d=>d.status === col).length})</h3>
                                <div className="space-y-3 p-2 rounded-lg bg-card/50 min-h-[60vh]">
                                    {upstreamInitiatives
