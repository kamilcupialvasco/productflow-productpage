...
const KeyResultItem: React.FC<{ kr: KeyResult }> = ({ kr }) => {
    ...
    <div className="p-3 bg-background/50 rounded-md">
        <div className="flex justify-between items-center mb-2">
            <p className="text-sm text-text-primary flex-1">{kr.text}</p>
        ...
        {linkedHypotheses.length > 0 && (
            <div className="mt-2 text-xs">
                ...
                <span className="text-text-secondary">{linkedHypotheses.map(h => `"${h.title}"`).join(', ')}</span>
            </div>
        )}
    </div>
)};
    ...
    return (
        <div className="bg-card rounded-lg border border-border p-6">
            <div className="flex justify-between items-start mb-4">
                <div>
                    <h3 className="text-xl font-semibold text-text-primary">{objective.title}</h3>
                    {linkedPillar && <p className="text-xs text-primary font-medium mt-1">Supports Pillar: {linkedPillar.title}</p>}
                </div>
        ...
        <div className="flex-1 flex flex-col overflow-y-auto">
            ...
            <div className="p-8">
                 <div className="flex justify-end mb-4">
                    <button className="px-4 py-2 rounded-md bg-primary text-white hover:bg-primary-hover">New Objective</button>
                </div>
