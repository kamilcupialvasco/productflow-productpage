...
const IdeaCard: React.FC<{ idea: Idea, onSelect: () => void, isSelected: boolean }> = ({ idea, onSelect, isSelected }) => (
    <div 
        onClick={onSelect}
        className={`p-4 border-l-4 cursor-pointer transition-colors ${isSelected ? 'bg-primary/10 border-primary' : 'bg-card/50 border-border hover:bg-card'}`}
    >
        <div className="flex justify-between items-start">
            <h4 className="font-semibold text-text-primary">{idea.title}</h4>
            <StatusBadge status={idea.status} />
        </div>
        <p className="text-xs text-text-secondary mt-1">Source: {idea.source}</p>
    </div>
);
    ...
    <div className="p-6 bg-card/30 rounded-lg">
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm p-3 bg-background rounded-md">{idea.aiSummary || 'AI analysis pending...'}</p>
            </div>
             <div>
                <h4 className="font-semibold text-primary mb-2">Potential Risks (AI Generated)</h4>
                {idea.aiRisks ? (
                    <ul className="list-disc list-inside text-sm space-y-1">
                        {idea.aiRisks.map((risk, i) => <li key={i}>{risk}</li>)}
                    </ul>
                ) : <p className="text-sm p-3 bg-background rounded-md">AI analysis pending...</p>}
            </div>
             <div>
                <h4 className="font-semibold text-primary mb-2">Next Steps</h4>
                 <div className="flex flex-wrap gap-2">
                    <button onClick={() => navigateTo('upstream/initiatives')} className="text-sm px-3 py-1.5 rounded-md bg-border hover:bg-zinc-600">Create Initiative</button>
                    <button className="text-sm px-3 py-1.5 rounded-md bg-border hover:bg-zinc-600">Link to Existing</button>
            ...
            <div className="flex-1 flex p-8 gap-8 overflow-hidden">
                <div className="w-1/3 flex-shrink-0 flex flex-col">
                     <div className="flex justify-between items-center mb-4">
                        ...
                        <button className="px-3 py-1.5 text-sm rounded-md bg-primary text-white hover:bg-primary-hover">Add Idea</button>
                    </div>
                ...
                <div className="w-2/3 flex-1 overflow-y-auto">
                    {selectedIdea ? (
                        ...
                        <div className="flex items-center justify-center h-full">
                            <p className="text-text-secondary">Select an idea to view details</p>
                        </div>
                    )}
                </div>
            </div>
