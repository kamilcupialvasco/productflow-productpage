    ...
    return (
        <div className="flex-1 flex flex-col">
            <div className="p-8 flex-1 flex gap-8">
                    ...
                    <nav className="space-y-1">
                        {articles.map(article => (
                            <button 
                                ...
                                className={`w-full text-left px-3 py-2 rounded-md text-sm ${selectedArticle.id === article.id ? 'bg-vasco-primary/20 text-vasco-primary' : 'hover:bg-vasco-dark-card'}`}
                            >
                                {article.title}
                            </button>
                        ))}
                ...
                <main className="w-3/4 bg-vasco-dark-card rounded-lg border border-vasco-dark-border p-6">
                    <h2 className="text-2xl font-bold mb-4">{selectedArticle.title}</h2>
                    <div className="prose prose-invert max-w-none prose-p:text-vasco-text-secondary prose-headings:text-vasco-text-primary prose-strong:text-vasco-text-primary">
                        <p>This is a mock documentation page. In a real application, this content could be written in Markdown and rendered here.</p>
                        <pre className="bg-vasco-dark-bg p-4 rounded-md text-sm">
                            <code>
