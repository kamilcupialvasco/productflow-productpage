    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center">
            <div className="bg-vasco-dark-card rounded-lg w-full max-w-lg">
                <div className="p-4 border-b border-vasco-dark-border"><h3>{context ? 'Edit' : 'Add'} Context</h3></div>
                <div className="p-6 space-y-4">
                    <input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Context Name" className="w-full bg-vasco-dark-bg p-2 rounded"/>
                    <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="Description..." className="w-full bg-vasco-dark-bg p-2 rounded h-24"/>
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {mockPersonas.map(p => (
                                <button 
                                    ...
                                    className={`px-3 py-1.5 text-xs rounded-full border ${formData.linkedPersonaIds.includes(p.id) ? 'bg-vasco-primary border-vasco-primary text-white' : 'border-vasco-dark-border hover:bg-vasco-dark-border'}`}
                                >
                                    {p.name}
                                </button>
                            ))}
        ...
        <div className="flex-1 flex flex-col overflow-y-auto">
            ...
            <div className="p-8">
                <div className="flex justify-end mb-6">
                    <button onClick={() => setIsAdding(true)} className="px-4 py-2 rounded-md bg-vasco-primary text-white">Add Context</button>
                </div>
                <div className="space-y-4">
                    {contexts.map(ctx => (
                        <div key={ctx.id} className="p-4 bg-vasco-dark-card rounded-lg border border-vasco-dark-border flex justify-between items-start">
                           <div>
                             <h3 className="font-semibold text-lg text-vasco-text-primary">{ctx.name}</h3>
                             <p className="text-sm text-vasco-text-secondary mt-1">{ctx.description}</p>
                             <div className="mt-3 flex flex-wrap gap-2">
                                {ctx.linkedPersonaIds.map(pId => {
                                    const persona = mockPersonas.find(p => p.id === pId);
                                    return <span key={pId} className="px-2 py-1 text-xs rounded-full bg-vasco-dark-bg flex items-center">
                           ...
                           <button onClick={() => setEditingContext(ctx)} className="text-sm text-vasco-primary hover:underline">Edit</button>
                        </div>
                    ))}
                </div>
            </div>
