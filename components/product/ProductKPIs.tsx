...
const ChartContainer: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <div className="bg-vasco-dark-card p-6 rounded-lg border border-vasco-dark-border transition-shadow hover:shadow-xl">
        <h4 className="text-lg font-semibold text-vasco-text-primary mb-4">{title}</h4>
...
const KPIStatCard: React.FC<{ kpi: { name: string; value: string; target: string; trend: 'up' | 'down' | 'stable' } }> = ({ kpi }) => {
    ...
    return (
        <div className="bg-vasco-dark-card p-6 rounded-lg border border-vasco-dark-border text-center">
            <p className="text-sm text-vasco-text-secondary">{kpi.name}</p>
            <p className={`text-4xl font-bold my-2 ${color}`}>{kpi.value}</p>
            <div className="flex justify-center items-center text-xs text-vasco-text-secondary">
    ...
    if (!isHardware(product) || !product.kpis) {
        return (
             <div className="flex-1 flex flex-col">
                <header className="flex-shrink-0 h-16 bg-vasco-dark-card border-b border-vasco-dark-border flex items-center px-8">
                    <button onClick={onBack} className="mr-4 p-2 rounded-full hover:bg-vasco-dark-bg">&larr;</button>
                    <div>
                        <h2 className="text-xl font-semibold text-vasco-text-primary">Product KPIs</h2>
                        <p className="text-sm text-vasco-text-secondary">No KPIs defined for {product.name}</p>
                    </div>
                </header>
    ...
    return (
        <div className="flex-1 flex flex-col overflow-y-auto">
             <header className="flex-shrink-0 h-16 bg-vasco-dark-card border-b border-vasco-dark-border flex items-center px-8">
                ...
                <div>
                    <h2 className="text-xl font-semibold text-vasco-text-primary">Product KPIs</h2>
                    <p className="text-sm text-vasco-text-secondary">Performance metrics for {product.name}</p>
                </div>
            </header>
                    ...
                    <ChartContainer title="Sales (Units per Month)">
                        <ResponsiveContainer>
                            <LineChart data={kpis.sales} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                                ...
                                <Legend />
                                <Line type="monotone" dataKey="units" stroke="#4f46e5" strokeWidth={2} />
                            </LineChart>
                        </ResponsiveContainer>
                    </ChartContainer>
