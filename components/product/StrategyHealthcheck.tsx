...
const ChecklistItem: React.FC<{ item: typeof checklistItems[0] }> = ({ item }) => {
    ...
    return (
        <div className="flex items-start space-x-3 p-3 bg-vasco-dark-bg/50 rounded-md">
            <input type="checkbox" checked={checked} onChange={() => setChecked(!checked)} className="mt-1 h-4 w-4 rounded bg-vasco-dark-bg border-vasco-dark-border text-vasco-primary focus:ring-vasco-primary" />
            <label className="flex-1">
                <p className={`text-sm ${checked ? 'text-vasco-text-secondary line-through' : 'text-vasco-text-primary'}`}>{item.question}</p>
                <p className="text-xs text-vasco-text-secondary">{item.category}</p>
            </label>
        </div>
    )
}
        ...
        <div className="flex-1 flex flex-col overflow-y-auto">
            ...
            <div className="p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1">
                     <section className="bg-vasco-dark-card rounded-lg border border-vasco-dark-border p-6 sticky top-8">
                        <h3 className="text-xl font-semibold mb-4">AI Strategy Assistant</h3>
                        <p className="text-sm text-vasco-text-secondary mb-4">Let AI analyze your current data to generate critical questions you might be missing.</p>
                        <button onClick={generateAiQuestions} className="w-full px-4 py-2 rounded-md bg-vasco-primary text-white hover:bg-vasco-primary-hover transition">Generate Questions</button>
