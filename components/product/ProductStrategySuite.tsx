...
const ExpandableSection: React.FC<{title: string, statement: string, explanation: string}> = ({title, statement, explanation}) => {
    ...
    return (
        <div className="p-4 bg-vasco-dark-bg/50 rounded-lg">
            <div onClick={() => setIsOpen(!isOpen)} className="cursor-pointer flex justify-between items-center">
                <div>
                    <h3 className="text-lg font-semibold text-vasco-primary">{title}</h3>
                    <p className="text-vasco-text-primary">{statement}</p>
                </div>
                <span className="text-xl">{isOpen ? '−' : '+'}</span>
            </div>
            {isOpen && (
                <div className="mt-3 pt-3 border-t border-vasco-dark-border">
                    <h4 className="text-sm font-semibold text-vasco-text-secondary mb-1">What we understand by that:</h4>
                    <p className="text-sm text-vasco-text-secondary leading-relaxed">{explanation}</p>
                </div>
            )}
        </div>
    );
...
const PillarCard: React.FC<{pillar: StrategicPillar}> = ({pillar}) => {
     const [isOpen, setIsOpen] = useState(false);
    return (
        <div className="p-4 bg-vasco-dark-bg rounded-md">
            <div onClick={() => setIsOpen(!isOpen)} className="cursor-pointer">
                <h4 className="font-semibold text-vasco-primary">{pillar.title}</h4>
                <p className="text-sm text-vasco-text-secondary mt-1">{pillar.description}</p>
            </div>
             {isOpen && (
                <div className="mt-3 pt-3 border-t border-vasco-dark-border">
                     <div className="prose prose-invert prose-sm max-w-none p-2 bg-vasco-dark-bg/50 rounded-md border border-vasco-dark-border">
    ...
    return (
        <div className="flex-1 flex flex-col overflow-y-auto">
            ...
            <div className="p-8 space-y-6">
                 {/* Top Level */}
                 <div className="p-6 bg-vasco-dark-card/50 rounded-lg border border-vasco-dark-border space-y-4">
                ...
                <div className="space-y-4">
                    <div className="p-6 bg-vasco-dark-card/50 rounded-lg border border-vasco-dark-border">
                    ...
                    <div className="flex justify-center h-8 items-center"><div className="w-px h-full bg-vasco-dark-border"></div></div>
                    ...
                    <div className="p-6 bg-vasco-dark-card/50 rounded-lg border border-vasco-dark-border">
                        ...
                        {objectives.map(obj => {
                            ...
                            return (
                                <div key={obj.id} className="p-4 bg-vasco-dark-bg rounded-md border-l-4 border-vasco-primary">
                                    <p className="text-xs text-vasco-primary font-semibold">{linkedPillar?.title}</p>
                                    <h4 className="font-semibold text-vasco-text-primary">{obj.title}</h4>
                                    {obj.keyResults.map(kr => {
                                        ...
                                        return (
                                            <div key={kr.id} className="mt-3">
                                                ...
                                                <div className="w-full bg-vasco-dark-border rounded-full h-1.5"><div className="bg-green-500 h-1.5 rounded-full" style={{ width: `${kr.progress}%` }}></div></div>
                                                {linkedInitiatives.length > 0 && (
                                                    <div className="mt-2 text-xs flex items-center space-x-2">
                                                        <span>🔗</span>
                                                        {linkedInitiatives.map(li => <span key={li.id} className="bg-vasco-dark-border px-1.5 py-0.5 rounded-full">{li.title}</span>)}
                                                    </div>
                                                )}
                                            </div>
                                        )
