...
const IntegrationStatusCard: React.FC<{ item: IntegrationStatus }> = ({ item }) => (
    <div className="flex items-center justify-between p-3 bg-vasco-dark-bg/50 rounded-md">
        <div>
            <p className="font-medium text-sm">{item.id}</p>
            <p className="text-xs text-vasco-text-secondary">Last sync: {item.lastSync}</p>
        </div>
        <div className="text-right">
            <StatusIndicator status={item.status} />
            <p className="text-xs text-vasco-text-secondary">{item.newInsights} new insights</p>
        </div>
    </div>
);
...
const AnomalyCard: React.FC<{ item: Anomaly; onClick: () => void }> = ({ item, onClick }) => {
    ...
    return (
        <div onClick={onClick} className={`p-4 rounded-lg border-l-4 cursor-pointer hover:bg-vasco-dark-bg ${severityClasses[item.severity]}`}>
            <h4 className="font-semibold text-vasco-text-primary">{item.title}</h4>
            <p className="text-sm text-vasco-text-secondary mt-1">{item.description}</p>
        </div>
    );
};
                ...
                <div className="lg:col-span-2 space-y-8">
                    <section className="bg-vasco-dark-card rounded-lg border border-vasco-dark-border p-6">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={startTour} className="text-sm text-vasco-primary hover:underline">Start Tutorial</button>
                        </div>
                        <div className="h-64">
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={trendData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                                    ...
                                    <Line type="monotone" dataKey="count" name="New Insights" stroke="#4f46e5" strokeWidth={2} dot={false} />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                    </section>
                    ...
                    >
                        <section id="home-anomalies" className="bg-vasco-dark-card rounded-lg border border-vasco-dark-border p-6">
                     ...
                     <section className="bg-vasco-dark-card rounded-lg border border-vasco-dark-border p-6">
                        ...
                        <div className="space-y-4">
                            <div className="text-center">
                                <p className="text-4xl font-bold text-vasco-primary">{last24hInsights.length}</p>
                                <p className="text-sm text-vasco-text-secondary">New Insights (24h)</p>
                            </div>
                             <div className="text-center">
                                <p className="text-4xl font-bold text-red-400">{last24hInsights.filter(i => i.sentiment === Sentiment.Negative).length}</p>
                                <p className="text-sm text-vasco-text-secondary">Negative Insights (24h)</p>
                            </div>
                        </div>
                    </section>
                </div>
