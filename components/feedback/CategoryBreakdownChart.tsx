...
const CategoryBreakdownChart: React.FC<CategoryBreakdownChartProps> = ({ data, onBarClick }) => {
  ...
  return (
    <div className="bg-card p-6 rounded-lg border border-border h-full">
      <h3 className="text-lg font-semibold text-text-primary mb-4">Insights by Category</h3>
      <div style={{ width: '100%', height: 300 }}>
        <ResponsiveContainer>
          <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 20, left: 30, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#3f3f46" />
            <XAxis type="number" stroke="#a1a1aa" />
            <YAxis dataKey="name" type="category" stroke="#a1a1aa" width={100} tick={{fontSize: 12}} />
            <Tooltip
                cursor={{fill: '#2d3748'}}
                contentStyle={{ 
